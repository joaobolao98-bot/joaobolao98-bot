...
function fazerLogin() {
    ...
    if ((user === "alc15anos" || user === "alice") && pass === "561564") {
        fecharLogin();
        localStorage.setItem('adminLogadoAlice', 'true');
        ...
        carregarCartoes();
        carregarVaralFotos();
        alert("Login efetuado com sucesso! Bem-vinda, Alice 👑 (Agora os botões de exclusão estão visíveis para você)");
    } else {
...
function fazerLogout() { 
    localStorage.removeItem('adminLogadoAlice');
    fecharPainelAdmin(); 
    carregarCartoes();
    carregarVaralFotos();
...
function carregarCartoes() {
    ...
    let isAdmin = localStorage.getItem('adminLogadoAlice') === 'true';
    lista.innerHTML = "";
    
    cartoes.forEach((c, index) => {
        let botaoExcluirHtml = isAdmin ? `<button class="btn-excluir" onclick="removerCartao(${index})" title="Excluir cartão">✕</button>` : '';
        lista.innerHTML += `
...
function carregarVaralFotos() {
    ...
    fotosDuplicadas.forEach((f, index) => {
        ...
        let botaoExcluirHtml = isAdmin ? `<button class="btn-excluir" onclick="removerFotoVaral(${realIndex}); event.stopPropagation();" title="Excluir foto">✕</button>` : '';
